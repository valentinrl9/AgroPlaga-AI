...
class _RegisterTreatmentScreenState extends State<RegisterTreatmentScreen> {
  ...
  Future<void> _loadBiocides() async {
    final plague = widget.scan?.plague ?? "tuta absoluta";
    final crop = widget.scan?.crop ?? "tomate";
    try {
      final list = await _repo.fetchBiocides(plague: plague, crop: crop);
      if (!mounted) return;
      setState(() {
        _biocides = list;
        _loading = false;
          ...
          : SingleChildScrollView(
              ...
              child: Column(
                ...
                children: [
                  if (widget.scan != null)
                    Text(
                      "Tras escaneo: ${widget.scan!.plague} · ${widget.scan!.crop}",
                  ...
                  if (_biocides.isNotEmpty)
                    DropdownButtonFormField<String>(
                      ...
                      onChanged: (v) async {
                        setState(() {
                          _selectedRegistry = v;
                          final match = _biocides.cast<Map<String, dynamic>>().firstWhere((b) => b["registry_no"] == v);
